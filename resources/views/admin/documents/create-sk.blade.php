        ...
        <div class="card-body">
            <form action="{{ route('admin.documents.sk.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('DIR') is-invalid @enderror" id="DIR" name="DIR" required>
                            ...
                            <option value="KPTS" {{ old('DIR') == 'KPTS' ? 'selected' : '' }}>KPTS</option>
                            <option value="DIR" {{ old('DIR') == 'DIR' ? 'selected' : '' }}>DIR</option>
                            <option value="SK" {{ old('DIR') == 'SK' ? 'selected' : '' }}>SK</option>
                        </select>
                        @error('DIR')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted">Digunakan untuk generate nomor SK</small>
                    </div>
                ...
                <div class="alert alert-info">
                    ...
                    <strong>Informasi:</strong> Dokumen yang dibuat oleh admin akan otomatis ter-approve dan nomor SK akan digenerate otomatis berdasarkan DIR dan tanggal.
                </div>
