    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="text-white mb-0"><i class="bi bi-file-earmark-plus"></i> Tambah Surat Addendum</h2>
                    <p class="text-white-50 mb-0">Buat Addendum baru (bisa backdate)</p>
                </div>
                <a href="{{ route('addendum.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
            </div>
        </div>
    </div>
    <div class="card">
        <div class="card-header"><h5 class="mb-0">Form Surat Addendum</h5></div>
        <div class="card-body">
            <form action="{{ route('addendum.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control" name="TANGGAL" value="{{ old('TANGGAL', date('Y-m-d')) }}" required>
                    <small class="text-muted">Addendum bisa backdate</small>
                </div>
                <div class="mb-3">
                    <label class="form-label">Nomor Perjanjian Asal <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="NOMOR_PERJANJIAN_ASAL" value="{{ old('NOMOR_PERJANJIAN_ASAL') }}" placeholder="Contoh: -001/SP/DIR/2024" required>
                    <small class="text-muted">Nomor SP yang akan di-addendum</small>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="PERIHAL" rows="3" placeholder="Jelaskan perihal addendum" required>{{ old('PERIHAL') }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="form-label">Perubahan <span class="text-danger">*</span></label>
                    <textarea class="form-control" name="PERUBAHAN" rows="4" placeholder="Jelaskan perubahan dari perjanjian asal" required>{{ old('PERUBAHAN') }}</textarea>
                </div>
                <div class="mb-3">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('addendum.index') }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
