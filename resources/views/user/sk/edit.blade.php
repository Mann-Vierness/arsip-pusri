...
<div class="container-fluid">
    <h2 class="mb-4"><i class="bi bi-pencil"></i> Edit Surat Keputusan</h2>
    <div class="card">
        <div class="card-body">
            <div class="alert alert-warning">
                <i class="bi bi-exclamation-triangle"></i> Nomor SK: <strong>{{ $document->NOMOR_SK }}</strong> (tidak dapat diubah)
            </div>
            <form action="{{ route('user.sk.update', $document->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('user.sk.index') }}" class="btn btn-secondary"><i class="bi bi-x-circle"></i> Batal</a>
                </div>
            </form>
        </div>
    </div>
