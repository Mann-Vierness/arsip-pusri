...
@section('title', 'Edit Surat Perjanjian')
@section('content')
<div class="container-fluid">
    <h2 class="mb-4"><i class="bi bi-pencil"></i> Edit Surat Perjanjian</h2>
    <div class="card">
        <div class="card-body">
            <div class="alert alert-warning">
                <i class="bi bi-exclamation-triangle"></i> Nomor SK: <strong>{{ $document->NO }}</strong> (tidak dapat diubah)
            </div>
            <form action="{{ route('user.sp.update', $document->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('user.sp.index') }}" class="btn btn-secondary"><i class="bi bi-x-circle"></i> Batal</a>
                </div>
            </form>
        </div>
    </div>
